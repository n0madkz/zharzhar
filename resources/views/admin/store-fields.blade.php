...
@elseif(($field['preview'] ?? null) === 'video' && data_get($record?->config_json, 'video_url'))<video controls preload="metadata" src="{{ str_starts_with(data_get($record->config_json, 'video_url'), '/') ? asset(ltrim(data_get($record->config_json, 'video_url'), '/')) : data_get($record->config_json, 'video_url') }}" style="width:100%;max-width:260px;aspect-ratio:9/16;object-fit:cover;margin-top:8px;border-radius:18px"></video><small>Если файл не выбран, сохранится текущее видео.</small>
