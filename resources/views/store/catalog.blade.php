@php
    ...
    $copy = $kk ? [
        ...
        'choose' => 'Шақыруды таңдау', 'from' => 'бастап', 'payment' => 'Бір мереке. Бір төлем.',
        ...
        'choose' => 'Выбрать приглашение', 'from' => 'от', 'payment' => 'Один праздник. Одна оплата.',
    ...
    $steps = $kk ? [
        ...
        ['Kaspi арқылы төлеңіз', 'Көрсетілген нөмірге аударып, WhatsApp батырмасы арқылы тапсырыс нөмірін жіберіңіз.'],
        ...
        ['Оплатите через Kaspi', 'Переведите сумму и отправьте номер заказа через готовое сообщение в WhatsApp.'],
    ...
    $faqs = $kk ? [
        ...
        ['Қалай төлеуге болады?', 'Kaspi аударымын '.config('store.kaspi_phone').' нөміріне жібересіз. Нақты сома тапсырыс рәсімделгеннен кейін көрсетіледі.'],
        ...
        ['Как оплатить?', 'Переведите сумму через Kaspi на '.config('store.kaspi_phone').'. Точная сумма появится после оформления.'],
...
<div class="hero-art" aria-label="{{ $copy['art_label'] }}"><span class="orbit-label">{{ $copy['moments'] }}</span><div class="paper theme-sage"><span class="paper-kicker">ҮЙЛЕНУ ТОЙҒА ШАҚЫРУ</span><span class="ornament" aria-hidden="true">❧</span><span class="paper-script">Алихан<br><i>&</i> Аружан</span><span class="paper-date">12 · 09 · 2026</span><span class="paper-footer">БІЗДІҢ ҚУАНЫШЫМЫЗҒА ОРТАҚ БОЛЫҢЫЗ</span></div><span class="art-tag">{{ $copy['love'] }}</span></div>
</section>
...
<a href="{{ route('store.preview', $template) }}" class="design-preview invite-card-{{ $template->config_json['theme'] ?? 'sage' }} theme-{{ $template->config_json['theme'] ?? 'sage' }}" aria-label="{{ $copy['view'] }}: {{ $template->name }}">
@if($template->preview_image)<img src="{{ $template->preview_image }}" alt="" loading="lazy">@endif
<span class="card-shade" aria-hidden="true"></span><span class="mini-kicker">{{ $template->event_type === 'qyz_uzatu' ? 'ҚЫЗ ҰЗАТУ' : ($template->event_type === 'anniversary' ? 'МЕРЕЙТОЙ' : 'БІЗ ҮЙЛЕНЕМІЗ') }}</span><span class="card-ring" aria-hidden="true"></span><strong>{{ $template->config_json['sample_names'] ?? ($template->event_type === 'qyz_uzatu' ? 'Аружан' : ($template->event_type === 'anniversary' ? '60 жас' : 'Алихан & Аружан')) }}</strong><span class="mini-date">12 / 09 / 2026</span>
<span class="preview-pill">{{ $copy['view'] }} ↗</span></a>
<div class="design-info"><div><p class="eyebrow">{{ $template->event_type ? ($eventLabels[$template->event_type] ?? $template->event_type) : $copy['any_event'] }}</p><h3>{{ $template->name }}</h3></div><strong class="design-price">{{ number_format($template->price, 0, ',', ' ') }} ₸</strong></div><a class="button outline full" href="{{ route('store.checkout', $template) }}">{{ $copy['select_design'] }} <span>→</span></a>
</article>@empty<div class="empty-state"><h3>{{ $copy['empty'] }}</h3><p>{{ $copy['empty_text'] }}</p><a href="tel:{{ preg_replace('/[^+0-9]/', '', config('store.kaspi_phone')) }}">{{ $copy['contact'] }}</a></div>@endforelse</div></section>
