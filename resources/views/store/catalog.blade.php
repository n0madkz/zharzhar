...
<span class="card-shade" aria-hidden="true"></span><span class="mini-kicker">{{ $template->config_json['content_kk']['event_label'] ?? ($template->event_type === 'qyz_uzatu' ? 'ҚЫЗ ҰЗАТУ' : ($template->event_type === 'anniversary' ? 'МЕРЕЙТОЙ' : 'ҮЙЛЕНУ ТОЙЫ')) }}</span><span class="card-ring" aria-hidden="true"></span><strong>{{ $template->name }}</strong><span class="mini-date">{{ \Carbon\Carbon::parse($template->config_json['content_kk']['event_date'] ?? '2026-11-08')->format('d / m / Y') }}</span>
