@php
    ...
    $t = $kk ? [
        ...
        'archived' => 'Шақыру архивте', 'archived_hint' => 'Мерекеден кейін бір ай өтті. Қонақтарға арналған шақыру жабылды, ал жауаптар осы бетте сақталды.',
        ...
        'archived' => 'Приглашение в архиве', 'archived_hint' => 'После мероприятия прошёл месяц. Ссылка для гостей закрыта, а ответы сохранены на этой странице.',
...
<section class="panel"><p class="eyebrow">{{ $t['paid'] }}</p>@if($order->invitation->status === 'archived')<h3>{{ $t['archived'] }}</h3><p class="notice archive-notice">{{ $t['archived_hint'] }}</p>@else<h3>{{ $t['invite'] }}</h3><p>{{ $t['send_link'] }}</p><div class="copy-row"><input aria-label="{{ $t['invite_link'] }}" readonly value="{{ $order->publicUrl('i/'.$order->invitation->slug) }}"><button type="button" class="button outline" data-copy="{{ $order->publicUrl('i/'.$order->invitation->slug) }}">{{ $t['copy'] }}</button></div><p><a class="button primary" href="{{ $order->publicUrl('i/'.$order->invitation->slug) }}">{{ $t['open_invite'] }} ↗</a></p>@endif<h3>{{ $t['answers'] }}</h3><p class="hint">{{ $t['answers_hint'] }}</p><div class="copy-row"><input aria-label="{{ $t['answers_link'] }}" readonly value="{{ $order->publicUrl('responses/'.$order->responses_token) }}"><button type="button" class="button outline" data-copy="{{ $order->publicUrl('responses/'.$order->responses_token) }}">{{ $t['copy'] }}</button></div><p><a href="{{ $order->publicUrl('responses/'.$order->responses_token) }}">{{ $t['view_answers'] }} →</a></p></section>
